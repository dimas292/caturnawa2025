import { NextRequest, NextResponse } from 'next/server'
import { prisma } from '@/lib/prisma'

// Debug endpoint to check teams without auth
export async function GET(request: NextRequest) {
  try {
    const { searchParams } = new URL(request.url)
    const competitionType = searchParams.get('competitionType') // KDBI or EDC

    // Get registered teams for debate competitions
    const whereClause: any = {
      competition: {
        type: { in: ['KDBI', 'EDC'] }
      }
    }

    if (competitionType && ['KDBI', 'EDC'].includes(competitionType)) {
      whereClause.competition.type = competitionType
    }

    const registeredTeams = await prisma.registration.findMany({
      where: whereClause,
      include: {
        competition: {
          select: { 
            id: true,
            name: true, 
            type: true 
          }
        },
        participant: {
          select: { 
            id: true,
            fullName: true,
            email: true,
            university: true
          }
        },
        teamMembers: {
          select: {
            id: true,
            participantId: true,
            fullName: true,
            position: true
          },
          orderBy: { position: 'asc' }
        }
      },
      orderBy: [
        { competition: { type: 'asc' } },
        { teamName: 'asc' }
      ]
    })

    console.log(`Found ${registeredTeams.length} registered teams for debate competitions`)
    
    // Group teams by competition and status
    const teamsByCompetition = registeredTeams.reduce((acc, team) => {
      const compType = team.competition.type
      if (!acc[compType]) {
        acc[compType] = []
      }
      acc[compType].push({
        id: team.id,
        teamName: team.teamName,
        competitionId: team.competition.id,
        competitionName: team.competition.name,
        competitionType: team.competition.type,
        status: team.status,
        leader: team.participant,
        members: team.teamMembers,
        memberCount: team.teamMembers.length,
        university: team.participant.university
      })
      return acc
    }, {} as Record<string, any[]>)

    // Group by status
    const teamsByStatus = registeredTeams.reduce((acc, team) => {
      const status = team.status || 'UNKNOWN'
      if (!acc[status]) {
        acc[status] = []
      }
      acc[status].push({
        teamName: team.teamName,
        competition: team.competition.type,
        status: team.status
      })
      return acc
    }, {} as Record<string, any[]>)

    return NextResponse.json({
      success: true,
      data: { 
        teams: registeredTeams.map(team => ({
          id: team.id,
          teamName: team.teamName,
          competitionId: team.competition.id,
          competitionName: team.competition.name,
          competitionType: team.competition.type,
          status: team.status,
          leader: team.participant,
          members: team.teamMembers,
          memberCount: team.teamMembers.length,
          university: team.participant.university
        })),
        teamsByCompetition,
        teamsByStatus,
        stats: {
          KDBI: teamsByCompetition.KDBI?.length || 0,
          EDC: teamsByCompetition.EDC?.length || 0,
          total: registeredTeams.length,
          byStatus: Object.keys(teamsByStatus).reduce((acc, status) => {
            acc[status] = teamsByStatus[status].length
            return acc
          }, {} as Record<string, number>)
        }
      }
    })

  } catch (error) {
    console.error('Error fetching teams:', error)
    return NextResponse.json(
      { 
        error: 'Failed to fetch teams',
        details: error instanceof Error ? error.message : 'Unknown error'
      },
      { status: 500 }
    )
  }
}