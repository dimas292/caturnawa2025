...
/**
 * FORCE DELETE ALL EDC PRELIMINARY ROUNDS
 * Deletes all EDC PRELIMINARY rounds that don't have scores
 * 
 * Usage: POST /api/admin/force-delete-edc-rounds
 */
export async function POST() {
  ...
  try {
    const log: string[] = []
    log.push('🗑️  Force deleting all EDC PRELIMINARY rounds...\n')
    ...
    const msg = error instanceof Error ? error.message : 'Unknown error'
    console.error('Force delete EDC rounds error:', msg, error)
    return NextResponse.json({ 
