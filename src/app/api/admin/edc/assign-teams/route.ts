// src/app/api/admin/edc/assign-teams/route.ts
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Collect provided team IDs (non-empty) and ensure they belong to EDC and not duplicated in same round
    ...
    if (regs.length !== providedIds.length) {
      return NextResponse.json({ error: 'Some teams are invalid for EDC or not verified' }, { status: 400 })
    }
    ...
    console.error('EDC assign-teams error:', msg)
