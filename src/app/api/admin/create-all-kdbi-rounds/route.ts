...
/**
 * CREATE ALL KDBI ROUNDS WITH CORRECT MAPPING
 * Creates all 8 PRELIMINARY rounds for KDBI with proper roundNumber and session
 * 
 * Usage: POST /api/admin/create-all-kdbi-rounds
 */
export async function POST() {
  ...
  try {
    const log: string[] = []
    log.push('🔧 Creating all KDBI PRELIMINARY rounds with correct mapping...\n')
    ...
    const msg = error instanceof Error ? error.message : 'Unknown error'
    console.error('Create all KDBI rounds error:', msg, error)
    return NextResponse.json({ 
