// scripts/create-edc-tournament.js
// Script to create British Parliamentary tournament structure for EDC (English Debate Competition)
...
async function createEDCTournament() {
  ...
  try {
    // Get EDC competition
    const edcCompetition = await prisma.competition.findUnique({
      where: { type: 'EDC' }
    })

    if (!edcCompetition) {
      throw new Error('EDC competition not found. Please create it first.')
    }
